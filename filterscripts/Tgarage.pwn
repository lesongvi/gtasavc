...
public OnPlayerCommandText(playerid, cmdtext[]) {
	if(strcmp(cmdtext, "/cartune", true) == 0) {
     	if(IsPlayerInRangeOfPoint(playerid, 10,  -2458.4329,1831.6960,58.6881) || IsPlayerInRangeOfPoint(playerid, 10,  -166.4629,942.7173,15.0981) ||
	IsPlayerInRangeOfPoint(playerid, 10,  -1109.4095,803.7124,21.7562) || IsPlayerInRangeOfPoint(playerid, 10,  1027.7866,-1803.3795,1.2702) || IsPlayerInRangeOfPoint(playerid, 10,  1000.6925,-768.0753,1595.7860) || IsPlayerInRangeOfPoint(playerid, 10,  1012.8005,-738.1777,1602.9269) || IsPlayerInRangeOfPoint(playerid, 10,  1186.6995,370.1184,6.1022) || IsPlayerInRangeOfPoint(playerid, 10,  1016.1638,-793.0444,1610.8413) || IsPlayerInRangeOfPoint(playerid, 10,  1185.9215,370.8952,6.1013) ||
	IsPlayerInRangeOfPoint(playerid, 10,  1996.9829,-2223.2485,4.9267))
	    {
...
public ModCar(playerid) {
	switch(pmodelid[playerid]) {
        ...
        483,508,429,541,415,480,434,506,451,555,477,400,404,489,605,611,471,612,613,614,615,616,617,
		479,442,458,467,558: {
		    ShowMenuForPlayer(TuningMenu, playerid);
		    TogglePlayerControllable(playerid,0);
		    new vehicleid = GetPlayerVehicleID(playerid);
		    SetVehiclePos(vehicleid, 615.4673, -1.9374, 1000.6493);
		    SetPlayerVirtualWorld(playerid, 88);
		    SetVehicleVirtualWorld(vehicleid, 88);
		    LinkVehicleToInterior(vehicleid, 1);
		    SetVehicleZAngle(vehicleid, 91.7812);
		    SetPlayerCameraPos(playerid,610.362,0.246,1003.676);
			SetPlayerCameraLookAt(playerid,615.467,-1.937,1000.649);
